...
${colorConfig
  ...
  .map(([key]) => `  --color-${key}: ${config[key].color};`)
  .join("\n")}
}
`,
          )
...
interface TooltipContentProps {
  active?: boolean;
  payload?: Array<{ name?: string; value?: number; dataKey?: string; color?: string; fill?: string; payload?: Record<string, unknown> }>;
  label?: string;
      ...
      >
        ...
        <div className="grid gap-1.5">
          {payload.map((item, index) => {
            ...
            const itemConfig = getPayloadConfigFromPayload(config, item, key);
            const indicatorColor = color || item.payload?.fill as string || item.color || item.fill;
